...
| 构造和析构       | 调用                  | 不调用             |
| 分配内存位置     | 自由存储区            | 堆                 |
| 分配内存大小     | 编译器根据类型计算    | 显示指定字节数     |
| 返回类型         | 返回的是对象指针      | 返回 `void *`      |
| 内存分配失败返回 | 抛出 `std::bad_alloc` | 返回 `NULL`        |
| 对已分配内存扩充 | 无法直观处理          | 调用 `realloc`     |
| 分配内存不足     | 指定处理数或分配器    | 无法通过代码处理   |
| 处理数组         | 使用 `new[]`          | 计算数组大小后分配 |
| 函数重载         | 允许                  | 不允许             |
| 是否相互调用     | 可以具体看重载实现    | 不允许调用 `new`   |
